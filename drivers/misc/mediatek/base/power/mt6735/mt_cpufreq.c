...
#define CPU_DVFS_FREQ5_1p1 (702000)     /* KHz */
...
#endif
	struct device *dev;
#if defined(CONFIG_MT_ENG_BUILD) || !defined(DISABLE_PBM_FEATURE)
...
{
#ifdef CONFIG_DISABLE_SCREEN_OFF_DVFS_ENHANCEMENT
	cpufreq_info("@%s: LCM is %s, do nothing!\n", __func__, (onoff) ? "on" : "off");
#else
	struct mt_cpu_dvfs *p;
	int i;
	...
	if (onoff) {
		...
		for_each_cpu_dvfs(i, p) {
			if (!cpu_dvfs_is_available(p) || !p->dvfs_disable_by_early_suspend)
				continue;
